...
void ds::impl::DSRenderingHandler::operator()(core::TaskHandlerCondition cond) {       
    { util::XLockDisplayGuard guard(this->eng->getApplication()->display);
        ...
        glXMakeCurrent(this->eng->getApplication()->display, this->eng->getApplication()->win, this->eng->getApplication()->glc);
        glViewport(0, 0, this->eng->getApplication()->gwa.width, this->eng->getApplication()->gwa.height);

    }
    //std::cout << "Should render every : " << frameTime.count() << std::endl;
